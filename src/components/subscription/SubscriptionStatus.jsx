...
export default function SubscriptionStatus({ 
  ...
  const { subscription, usage, limits, isPremium, isCancelled, loading, error } = useSubscription();
        ...
        <div className="mb-4 text-sm text-gray-600">
          {isCancelled ? (
            <span className="text-yellow-600 font-medium">
              🗓️ Access until: {new Date(subscription.expiresAt).toLocaleDateString()}
            </span>
          ) : (
            <span>{isPremium ? 'Expires' : 'Expired'}: {new Date(subscription.expiresAt).toLocaleDateString()}</span>
          )}
        </div>
      )}
